...
### 2026-04-26 — grow reliability: sleep block, logging, 90-minute execution limit
Four real failure modes confirmed across Apr 21-23. Apr 21 and 22: empty session folders -- headed Playwright browser killed by laptop sleep mid-scout. Apr 23: 2 candidates, empty `drafts.json` -- transient `claude -p` failure (confirmed working in background today, exit 0). Apr 24: only day with a posted comment. Fixes shipped to `grow_run.py`: (1) RC-5 -- stderr redirected to `temporary/logs/grow_run_YYYY-MM-DD.log`; (2) RC-1 -- ctypes `SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED)` added to `main()`, released via atexit closure and normal return. `install_grow_scheduler.ps1`: ExecutionTimeLimit bumped 30 to 90 min, tasks re-registered. Diagnosis doc (`inbox/linkedin-agent/linkedin-grow-diagnosis-2026-04-25.md`) fact-checked: 4 inaccuracies noted -- atexit does not write an empty entry on crash (iterates empty list, adds nothing), ROOT is line 48 not 63, ExecutionTimeLimit is line 84 not 80, Apr 20 grow-2 explained by dry-run (not a failure). Verification due Mon 28 Apr: check `temporary/logs/grow_run_2026-04-28.log` for continuous output and `grow_log.json` for a new executed entry.
