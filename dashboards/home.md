...
const pages = dv.pages('"tasks/open"')
  .where(p => p.status === "open" || p.status === "in-progress")
  .sort(p => p.priority === "high" ? 0 : p.priority === "medium" ? 1 : 2);
const headers = ['Task', 'Repo', 'Project', 'Priority', 'Due', 'Status'];
dv.table(headers,
  pages.map(p => [
    dv.fileLink(p.file.path, false, p.title || p.file.name),
    p.repo, p.project, prioritySelect(p.file.path), p.due,
    statusSelect(p.file.path)
  ])
);
...
const pages = dv.pages('"tasks/open"')
  .where(p => p.due && p.due < dv.date("today") && p.status !== "done")
  .sort(p => p.due);
const headers = ['Task', 'Repo', 'Project', 'Priority', 'Due', 'Status'];
dv.table(headers,
  pages.map(p => [
...
const pages = dv.pages('"tasks/open"').where(p => p.status === "blocked");
const headers = ['Task', 'Repo', 'Project', 'Priority', 'Status'];
dv.table(headers,
  pages.map(p => [
    dv.fileLink(p.file.path, false, p.title || p.file.name),
    p.repo, p.project, prioritySelect(p.file.path),
    statusSelect(p.file.path)
  ])
);
scheduleDashboardColumnSort(dv, headers);
