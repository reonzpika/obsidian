...
const pages = dv.pages('"tasks/open"')
  .where(p => p.sprint === "2026-03-rd-sprint-1" && p.status !== "done")
...
dv.table(headers, pages.map(p => [
  ...
  p.owner, statusSelect(p.file.path), prioritySelect(p.file.path), p.due
]));
...
This sprint covers the first two weeks of the NexWave R&D programme (grant start: 12 March 2026, work commenced: 15 March 2026). Captured retroactively on 31 March 2026.

## What was completed

**Ryo**
- Employment contract drafted and signed
- R&D funding agreement signed (CONT-109091-N2RD-NSIWKC, 25 March 2026)
- Briefed accountant on NexWave R&D grant
- R&D operations system designed (Notion + Obsidian structure)
- LLM/RAG/fine-tuning initial research (started, ongoing into next sprint)

**Ting**
- Employment lawyer reviews — rounds 1 and 2
- Contacted accountant to initiate PAYE payroll setup
- Email Lisa Pritchard — confirmed Q1 time tracking and evidence requirements
- Found Indici contact and initiated sandbox access request
- R&D operations system designed (in collaboration with Ryo)

## Hours logged (MBIE evidence)
- Ryo: ~30hrs (Sat/Sun working pattern)
- Ting: ~80hrs (40hrs/week × 2 weeks, Mon–Fri)
