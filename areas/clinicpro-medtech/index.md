...
```dataviewjs
const active = dv.pages('"areas/clinicpro-medtech/projects"')
  .where(p => p.dashboard == "clinicpro-medtech" && p.status != "parked")
  .sort(p => p.title ?? p.file.name);
for (let p of active) {
  const badge = p.status == "production" ? "🟢" : "🔵";
  const taskCount = dv.pages('"areas/clinicpro-medtech/tasks/open"').where(t => t.project === p.id && t.status !== "done").length;
  const phase = p.phase ? `\n  _${p.phase}_` : "";
